...
fn with_unfilled_limit_order_should_set_vault_balance_to_zero() {
    ...
    let received_amount = get_swap_amount(&deps.as_ref(), &env, vault.clone())
        .unwrap()
        .amount;
    ...
    let original_offer_amount = get_swap_amount(&deps.as_ref(), &env, vault.clone())
        .unwrap()
        .amount;
...
fn with_partially_filled_limit_order_should_return_vault_balance_plus_retracted_amount() {
    ...
    let _original_offer_amount = get_swap_amount(&deps.as_ref(), &env, vault.clone())
        .unwrap()
        .amount;

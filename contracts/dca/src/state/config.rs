...
pub fn update_config(store: &mut dyn Storage, config: Config) -> StdResult<Config> {
    ...
    if config.automation_fee_percent > Decimal::percent(100) {
        return Err(StdError::generic_err(
            "automation_fee_percent must be less than 100%, and expressed as a ratio out of 1 (i.e. use 0.015 to represent a fee of 1.5%)",
        ));
    }

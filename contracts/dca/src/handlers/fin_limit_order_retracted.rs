...
use base::vaults::vault::VaultConfiguration;
            ...
            match vault.configuration {
                VaultConfiguration::DCA {
                    ...
                    let wasm_trade_event =
                        find_first_event_by_type(&fin_retract_order_response.events, "wasm")
                            .unwrap();
                    ...
                    if amount_retracted != limit_order_cache.original_offer_amount {
                        let retracted_balance = Coin {
                            ...
                            amount: vault.balances[0].amount - (swap_amount - amount_retracted),
                        };
